...
class TopicController extends Controller
{
    ...
    public function validatetopicurl(Request $request)
    {
        ...
        if(!is_null($request->url) && !is_null($request->id)){
            if(strlen($request->url) < 3) $result = 'short';
            elseif(in_array($request->url, $reserved)) $result = 'reserved';
            else {

                $topic = Topic::where('url', sn_urlify($request->url))->first();
                if(isset($topic->id)){
                    if($topic->id !== $request->id) $result = 'taken';
                    elseif($topic->id === $request->id && $topic->url === $request->url) $result = 'stet';
                    else $result = 'ok';
                }
                else $result = 'ok';
            }
    ...
    public function manage($do){
        ...
        if(sn_can('content')){
            ...
            if($do === 'redirects') return redirect()->route('be.redirects.index');
    ...
    public function create(Request $request)
    {
        ...
        if(!is_null($request->clone_id)){
            ...
            if(isset($original->id)){
                ...
                foreach($content as $key => $values) $content[$key]['value'] = '';
                $topic->content = $content;
                $metas = $topic->metas;
                foreach($metas as $key => $values) $metas[$key]['value'] = '';
                $topic->metas = $metas;
                $topic->url = null;
            }
        }
        ...
        $topic->save();
        if(!is_null($request->url_prefix)) $topic->url = $request->url_prefix.'/'.$topic->id;
        else $topic->url = 'new-'.strtolower(sn_config('topics.topics.'.$request->functionality.'.label')).'-'.$topic->id;
        $topic->save();
    ...
    public function edit($id) {
        ...
        if(isset($topic->content)){
            ...
            foreach($content as $field_name => $values){
                if(isset($content[$field_name]['type'])){
                    if($content[$field_name]['type'] === 'media'){
                        if(!isset($content[$field_name]['value'])) $content[$field_name]['value'] = [];
                        if(!isset($content[$field_name]['aft'])) $content[$field_name]['aft'] = 'jpg,png,gif';
                    }
                    elseif($content[$field_name]['type'] === 'text' || $content[$field_name]['type'] === 'richtext'){
                        if(!isset($content[$field_name]['value'])) $content[$field_name]['value'] = '';
                        if(!isset($content[$field_name]['max'])) $content[$field_name]['max'] = '999';
                    }
                }
            }

            foreach($content_fso as $k => $field_name){ if(!array_key_exists($field_name, $content)) unset($content_fso[$k]); }
            # dd($content_fso);

            foreach($content as $k => $v){ if(!in_array($k, $content_fso) && $k !== 'sn_fso') $content_fso[] = $k; }
            ...
            foreach($metas as $field_name => $values){
                if(isset($metas[$field_name]['type'])){
                    if($metas[$field_name]['type'] === 'media'){
                        if(!isset($metas[$field_name]['value'])) $metas[$field_name]['value'] = [];
                        if(!isset($metas[$field_name]['aft'])) $metas[$field_name]['aft'] = 'jpg,png,gif';
                    }
                    elseif($metas[$field_name]['type'] === 'text' || $metas[$field_name]['type'] === 'richtext'){
                        if(!isset($metas[$field_name]['value'])) $metas[$field_name]['value'] = '';
                        if(!isset($metas[$field_name]['max'])) $metas[$field_name]['max'] = '999';
                    }
                }
            }
            foreach($metas_fso as $k => $field_name){ if(!array_key_exists($field_name, $metas)) unset($metas_fso[$k]); }
            foreach($metas as $k => $v){ if(!in_array($k, $metas_fso) && $k !== 'sn_fso') $metas_fso[] = $k; }
            if($k = array_search('sn_fso', $metas_fso) !== false) unset($metas_fso[$k]);
            ...
            else $url_prefix = sn_config('topics.topics.'.$topic->functionality.'.public_url');
    ...
    public function save(UpdateTopicRequest $request)
    {
        ...
        try {
            if(isset($request->id)){
                ...
                else $message = "Could not find that topic.";
            }
            else $message = "No topic ID provided. Topic not updated.";
        }
        catch (Throwable $e) {
            $result = 'error';
    ...
    public function settings($id, $do = false){
        ...
        if($topic->parent === null) {
            if($do === false) $do = 'edit_topic_settings_parent';
            $parent = $topic;
            ...
            if($do === false) $do = 'edit_topic_settings_child';
    ...
    public function saveTopicStatus(UpdateTopicRequest $request)
    {
        ...
        try {
            ...
            else $message = "Missing data. Topic not updated.";
        }
        catch (Throwable $e) {
            $result = 'error';
            $message = $e->getMessage();
    ...
    public function destroyTopicField(Request $request)
    {
        ...
        if(isset($topic->id) && isset($data['field_to_destroy'])){
            $content = $topic->content;
            if(isset($content[$data['field_to_destroy']])) unset($content[$data['field_to_destroy']]);
            if(isset($content['sn_fso']) && ($key = array_search($data['field_to_destroy'], $content['sn_fso'])) !== false) unset($content['sn_fso'][$key]);
            $topic->content = $content;
            $topic->save();
            return response()->json([
                'result' => 'ok',
    ...
    public function saveTextFieldValue(Request $request)
    {
        ...
        if(isset($data['field_name']) && isset($data['topic_id'])){
            ...
            if(isset($topic->id)){
                ...
                # topic title
                if($request->field_type ==='topic_title') $topic->title = $v;
        ...
        if(isset($data) && is_array($data) & count($data) > 0 && isset($data[0]['topic_id'])){
            ...
            if(isset($topic->id)){
                ...
                foreach($data as $field){
                    if($field['field_type'] === 'topic_title') $topic->title = $field['field_value'];
                    elseif($field['field_type'] === 'content') $content[$field['field_name']]['value'] = $field['field_value'];
                    elseif($field['field_type'] === 'metas') $metas[$field['field_name']]['value'] = $field['field_value'];
                }
                $topic->content = $content;
                $topic->metas = $metas;
                $topic->save();
