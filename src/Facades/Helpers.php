...
class Helpers extends Facade
{
    ...
    public static function dash_topics($array, $l = 1){
        ...
        foreach ($array as $key => $value) {
            ...
            if(isset($value['a']['title'])){
                ...
                $output.= "<ul>";
                foreach(config('superniftycms.status.topics.values') as $s => $status) $output.= "<li data-v=\"{$s}\" data-l=\"{$status['label']}\"></li>";
                $output.= "</ul>";
                $output.= "</div>";
                $output.= "</li>";
    ...
    public static function get_selected_blade($selection_path)
    {
        ...
        if(is_file($full_path_to_blade.".blade.php")) return $full_path_to_blade;
        return null;
    }

    public static function blade_dot_path($path)
    {
        if(File::exists(resource_path('views/'.$path))) return str_replace('/', '.', $path);
        if(File::exists(resource_path('views/vendor/superniftycms/topics/'.$path))) return 'vendor.superniftycms.topics.'.str_replace('/', '.', $path);
        return config('superniftycms.layouts.auto');
    }

    public static function media_exists($media, $size = 'original')
    {
        if(isset($media->id) && Storage::disk(config('superniftycms.uploads.disk'))->exists(config('superniftycms.uploads.storage_directory').'/'.$media->id.'/'.$size.'.'.$media->type)) return true;
        return false;
    }
    ...
    public static function img_resize($media, $size = false)
    {
        ...
        if(!in_array($media->type, config('superniftycms.uploads.images.process'))) return null;
        ...
        try {
            ...
            elseif($size === 'thumbnail') { $size = config('superniftycms.uploads.images.thumb_width'); }
            elseif($size === 'poster') { $size = config('superniftycms.uploads.videos.poster_width'); }
    ...
    public static function media_upload_url($m, $size = false) {
        ...
        if(Helpers::media_exists($media)){
            ...
            if(in_array($media->type, config('superniftycms.uploads.images.process'))) {
                ...
                if(!Helpers::media_exists($media, $size) && Helpers::media_exists($media)) Helpers::img_resize($media, $size);
            ...
            elseif(in_array($media->type, config('superniftycms.uploads.videos.process'))) {
                if($size === 'poster') $path = "{$media->id}/poster.jpg";
    ...
    public static function media_upload_urls($media)
    {
        ...
        if(Helpers::media_exists($media)) {
            ...
            else $urls['original'] = Helpers::media_upload_url($media);
